...
def main():
    ...
    if args.prune:
        ...
        hook = FisherPruningHook(pruning=True, use_ista=True, start_from=args.prune)
...
def validate(val_loader, model, criterion, hook):
    ...
    ista_err = hook.ista_err.cpu().item()
    print(f' * Prec@1 {top1.avg:.3f}. Loss {losses.avg:.4f}. BinErr:', " ".join(format(x, ".3f") for x in hook.ista_err_bins))
